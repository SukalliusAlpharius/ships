        ...
        for ship_size in Game.ships_rules:
            ...
            while True:
                ...
                if player.auto_ship_setup is not True:
                    ...
                    player.message.append('''Куда поставить корабль размером в {} :
пример расстановки: [a 1 v или h] (v-вертикаль или h-горизонталь)
корабли размером в 1 клетку можно ставить только по вертикали
между кораблями должна быть одна пустая клетка'''.format(ship_size))
